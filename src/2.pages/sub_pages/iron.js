...
export default function Iron() {
    ...
    return (
        <div className="sub_page">
            <div className="head">
                <h2>ستائر المعدنية</h2>
                <p><Link to='/'>الرئيسية</Link> / <Link onClick={()=> setSelector('all')} className={"current"}>ستائر المعدنية</Link></p>
                
            </div>
            <div className="container">
                {curtains.filter((e) => e.type === "ستائر معدنية").map((el,i) => 
                <CardProduct
                    key={i}
                    id={el._id}
