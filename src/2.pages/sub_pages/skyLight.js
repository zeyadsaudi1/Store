...
export default function SkyLight() {
    ...
    return (
        <div className="sub_page">
            <div className="head">
                <h2>مظله اسكاي لايت</h2>
                <p><Link to='/'>الرئيسية</Link> / <Link onClick={()=> setSelector('all')} className={"current"}>مظله اسكاي لايت</Link></p>
                
            </div>
            <div className="container">
                {curtains.filter((e) => e.type === "مظله اسكاي لايت").map((el,i) => 
                <CardProduct
                    key={i}
                    id={el._id}
